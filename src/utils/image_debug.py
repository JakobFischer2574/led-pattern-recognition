...
def draw_detection_debug_image(
    ...
    for rx in locator_data.get("reconstructed_slots", []):
        cv2.line(base, (int(rx), 0), (int(rx), h - 1), (255, 0, 255), 2)
    ...
    draw_text(f"projection_peak_count: {locator_data.get('projection_peak_count', 0)}", y0); y0 += line_h
